  ...
  useEffect(() => {
    ...
    if (savedNewsJson !== null) {
      ...
      if (Array.isArray(parsedData)) {
        ...
        console.error('Parsed data from local storage is not an array:', parsedData);
      }
    }
  }, []);
            ...
            {savedNews.map((savedNewsItem, index) => (
                <li key={index} className="flex items-center justify-between gap-x-6 py-5">
                    <div className="min-w-0">
                        <a href={savedNewsItem.url} target="_blank" className="text-sm font-semibold leading-6 text-gray-900 hover:text-slate-400">{savedNewsItem.title}</a>
                        <div className="mt-1 flex items-center gap-x-2 text-xs leading-5 text-gray-500">
                            <p className="whitespace-nowrap">Author {savedNewsItem.author}</p>
                            <svg viewBox="0 0 2 2" className="h-0.5 w-0.5 fill-current">
                                <circle cx={1} cy={1} r={1} />
                            </svg>
                            <p className="truncate">News from {savedNewsItem.source.name}</p>
                        </div>
                    </div>
                </li>
            ))}
